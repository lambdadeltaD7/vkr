...
%	\newpage
	...
	\noindent Заметим(по приколу), что узлы удовлетворяют
	\[
	p_0'(x) = \frac{d}{dt} \left[\prod_{k=1}^{m} (t - x_k) \right]_{t=x} = \ 0
	\]
	
	\noindent Вспоминаем про первое (в смысле степени) уравнение системы. В операторном виде оно выглядит так:
	\[
	A(p) = 0
	\]
	где $A = A_1 - A_2, \ \  A_1(p) = \lambda^* p'(x), \ \ A_2(p) = \sum_{k=1}^{m} a_k p(x_k), \ \ p(t)=t$
	
	\noindent То есть 
	\[
	 	\lambda^* = \sum_{k=1}^{m} a_k x_k
	\]
	
	\noindent Теперь вспомним про второе (в смысле степени) уравнение системы.\\ Всё то же самое, но при $p(t)=t^2$ даёт нам
	\[
	2 \lambda^* x = \sum_{k=1}^{m} a_k x_k^2
	\]
	\[
	\lambda^* = \frac{1}{2x}\sum_{k=1}^{m} a_k x_k^2
	\]
	
	\noindent Комбинируя получаем квадратное относительно $x_2$\\
	\textbf{НО ВЕСА ТО НЕЗИВЕСТНЫ И ВЫРАЖАЮТСЯ ЧЕРЕЗ ИКСЫ ЛОЛ))) }
	\[
	\frac{1}{2x}\sum_{k=1}^{m} a_k x_k^2 = \sum_{k=1}^{m} a_k x_k
	\]
	...
	\noindent Очевидно, что
	\[
	\frac{d}{dt} \left[\prod_{k=1}^{m} (t - x_k) \right] =
	\sum_{i=1}^{m} \underset{k \neq i}{\prod_{k=1}^{m}}(t - x_k)
	\]
	\[
	\sum_{i=1}^{m} \underset{k \neq i}{\prod_{k=1}^{m}} \left( x - (\alpha_k x_{m-1} + \beta_k) \right) = 0
	\]
