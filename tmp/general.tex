...
\begin{document}
	...
	\subsection{СРАЗУ ОПИСАТЬ СДВИГ ВЛЕВО И ВПРАВА}
	\noindent Для поиска узлов используем
	\[
	p_0'(x) = \frac{d}{dt} \left[\prod_{k=1}^{m} (t - x_k) \right]_{t=x} = \ 0
	\]
	
	\noindent где $x_k = a t_k + b$ для неких $a, b \in \mathbb{R}$ \\
	а $t_k, \  k \in 1, \ldots, m$ есть точки альтернанса полинома чебышёва 1-го рода (выкинув последнюю)
	...
	\[
	t_k = \cos \left( \frac{(k-1) \pi}{m} \right), \ k \in 1 \ldots m
	\]
